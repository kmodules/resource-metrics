...
func init() {
	api.Register(schema.GroupVersionKind{
		...
		Version: "v1alpha2",
		Kind:    "Weaviate",
	}, Weaviate{}.ResourceCalculator())
}

type Weaviate struct{}

func (w Weaviate) ResourceCalculator() api.ResourceCalculator {
	return &api.ResourceCalculatorFuncs{
		AppRoles:               []api.PodRole{api.PodRoleDefault},
		RuntimeRoles:           []api.PodRole{api.PodRoleDefault},
		RoleReplicasFn:         w.roleReplicasFn,
		ModeFn:                 w.modeFn,
		UsesTLSFn:              w.usesTLSFn,
		RoleResourceLimitsFn:   w.roleResourceFn(api.ResourceLimits),
		RoleResourceRequestsFn: w.roleResourceFn(api.ResourceRequests),
	}
}

func (w Weaviate) roleReplicasFn(obj map[string]any) (api.ReplicaList, error) {
...
func (w Weaviate) roleResourceFn(fn func(rr core.ResourceRequirements) core.ResourceList) func(obj map[string]any) (map[api.PodRole]api.PodInfo, error) {
	return func(obj map[string]any) (map[api.PodRole]api.PodInfo, error) {
		container, replicas, err := api.AppNodeResourcesV2(obj, fn, WeaviateContainerName, "spec")
		if err != nil {
			return nil, err
		}
