...
func TestElasticsearchDashboard(t *testing.T) {
	type want struct {
		replicas       int64
		mode           string
		usesTLS        bool
	...
	tests := []struct {
		...
		{
			name: "testdata/elasticsearch.kubedb.com/v1alpha1/elasticsearchdashboard/dashboard.yaml",
			want: want{
				replicas: 3,
				mode:     ElasticsearchDashboardModeDefault,
				usesTLS:  true,
				totalResources: core.ResourceRequirements{
					Limits: core.ResourceList{
						core.ResourceCPU:    resource.MustParse("3"),
						core.ResourceMemory: resource.MustParse("4.35Gi"),
					},
					Requests: core.ResourceList{
						core.ResourceCPU:    resource.MustParse("1800m"),
						core.ResourceMemory: resource.MustParse("3.375Gi"),
					},
				},
				appResources: core.ResourceRequirements{
					Limits: core.ResourceList{
						core.ResourceCPU:    resource.MustParse("2400m"),
						core.ResourceMemory: resource.MustParse("3.6Gi"),
					},
					Requests: core.ResourceList{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := ElasticsearchDashboard{}.ResourceCalculator()
			...
			if got, err := c.UsesTLS(obj); err != nil {
				t.Errorf("UsesTLS() error = %v", err)
			} else if got != tt.want.usesTLS {
				t.Errorf("UsesTLS found = %v, expected = %v", got, tt.want.usesTLS)
			}
