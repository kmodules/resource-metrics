...
func TestVaultServer(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "testdata/kubevault.com/v1alpha2/vaultserver/vault.yaml",
			want: want{
				replicas: 3,
				mode:     VaultModeDefault,
				totalResources: core.ResourceRequirements{
		...
		{
			name: "testdata/kubevault.com/v1alpha2/vaultserver/vault2.yaml",
			want: want{
				replicas: 1,
				mode:     VaultModeDefault,
				totalResources: core.ResourceRequirements{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := VaultServer{}.ResourceCalculator()
